...
async function attachSocketServer(io) {
  ...
  io.on('connection', (socket) => {
    ...
    // Fix B: restore tournament entrant identity on reconnect (by deviceId only
    // — never creates a new entrant; only refreshes the live socketId so the
    // final-duel wiring in _startFinalDuel uses the correct socket).
    {
      const room = getRoom(roomId);
      if (room?.roomMode === 'TOURNAMENT' && room.tournament) {
        const ents = room.tournament.entrants;
        const existing = Object.keys(ents).find(
          (id) => ents[id].deviceId === socket.data.deviceId
        );
        if (existing) {
          ents[existing].socketId = socket.id; // refresh live socket
          socket.data.entrantId = existing;
          socket.emit('joined_as_entrant', { entrantId: existing });
        }
      }
    }
    ...
    socket.on('join_game', (payload, ack) => {
      ...
      if (result.ok) {
        if (result.slot) {
          ...
          // Record the live socket on the entrant so _startFinalDuel can wire
          // finalists directly into their duel slots (Fix A step 1).
          const room = getRoom(roomId);
          if (room?.tournament?.entrants[result.entrantId]) {
            room.tournament.entrants[result.entrantId].socketId = socket.id;
          }
    ...
    socket.on('prompt_typing', (payload) => {
      ...
      // Fix A step 5: additive deviceId fallback for finalists in FINAL_DUEL.
      // Normal players hit socket.data.slot first and are unaffected.
      const slot =
        socket.data.slot ||
        _identifySlot(socket) ||
        _identifySlotByDevice(roomId, socket.data.deviceId);
      if (!slot) return;
    ...
    socket.on('start_tournament', (payload, ack) => {
      ...
      // async, fire-and-forget; it broadcasts. .catch so a round failure can't
      // become a silent unhandledRejection that freezes the match.
      require('../game/tournament/lifecycle.js')
        .startTournament(roomId)
        .catch((e) => console.error(`[tournament] startTournament failed for ${roomId}:`, e && e.message));
      if (typeof ack === 'function') ack({ ok: true });
    });
