...
const { withTimeout, timeoutMs, retry } = require('../../async.js');
...
async function _runRound(roomId, epoch, targetSize) {
  ...
  // 1) Topic + hidden target image (retry/replay + "preparing" broadcast), THEN
  //    open the timed prompting window once a real target exists.
  await _prepareTopicAndReference(roomId, epoch);
  if (!isCurrentEpoch(roomId, epoch)) return;
  ...
  room.referenceImageUrl = null; // fresh target next round
  broadcastState(roomId);
...
async function _waveGenerate(roomId, epoch, ids) {
  ...
  for (const e of ents) {
    ...
    try {
      // 1v1 ile aynı desen: EN'e çevir → guard → üret → upload. Çeviri patlarsa
      // (yalnızca İngilizce-olmayan metinde olur) ham metni görsel modeline YOLLAMA
      // — dıştaki catch lastImageUrl=null yapar (entrant görselsiz, düşük skor).
      // Yanıltıcı/alakasız görsel üretmektense görselsiz kalsın.
      const promptText = await translateToEnglish(e.lastPrompt || '');
      if (!isCurrentEpoch(roomId, epoch)) return;
...
// Per-round/group hidden TARGET image. Mirrors the duel reference flow
// (matchLifecycle: generateImage(promptEn + SAFETY_SUFFIX)) — the topic is a
// TRUSTED system prompt, so it uses SAFETY_SUFFIX, NOT buildGuardedPrompt (that
// guards untrusted player text). DEMO_MODE → placeholder, no quota. On failure
// leaves referenceImageUrl null; scoreRoster then forfeits everyone to 0 rather
// than throwing. Epoch-guarded so a reset/skip mid-gen drops the stale result.
async function _generateReferenceImage(roomId, epoch) {
  const room = getRoom(roomId);
  if (!room || !room.tournament) return;
  const t = room.tournament;
  if (process.env.DEMO_MODE === '1') {
    // Real renderable placeholder (the URL is now shown as an <img>), mirroring
    // the duel demo path — NOT a demo:// scheme that renders as a broken image.
    room.referenceImageUrl = `https://picsum.photos/seed/pc-tref-${t.roundIndex}/640`;
    return;
  }
  const promptEn = (t.topic && t.topic.prompt) || '';
  if (!promptEn) { if (isCurrentEpoch(roomId, epoch)) room.referenceImageUrl = null; return; }
  try {
    const limit = timeoutMs('TOURNAMENT_GEN', 20000);
    // Retry transient provider failures (402 burst / timeout) before giving up —
    // a single blip must not forfeit the whole round.
    const { buffer, mimeType } = await retry(
      () => withTimeout(generateImage(promptEn + SAFETY_SUFFIX), limit, 'tref'),
      { attempts: 3, label: 'tref' }
    );
    if (!isCurrentEpoch(roomId, epoch)) return;
    const ext = mimeType.includes('jpeg') ? 'jpg' : 'png';
    const url = await uploadBuffer(`tournament/${roomId}/ref_r${t.roundIndex}.${ext}`, buffer, mimeType);
    if (!isCurrentEpoch(roomId, epoch)) return;
    room.referenceImageUrl = url;
  } catch (err) {
    // Guard the failure write too: a stale (post-reset) gen that rejects must not
    // clobber the CURRENT epoch's reference.
    if (!isCurrentEpoch(roomId, epoch)) return;
    room.referenceImageUrl = null;
    log(roomId, `ref gen fail: ${err.message}`);
  }
}

// Pick a topic and generate its hidden target image, retrying with FRESH topics
// until a reference exists or attempts are exhausted. Broadcasts a "preparing"
// state first (ROUND_PROMPTING + PROMPTING phase with NO countdown, reference
// still null) so clients leave the prior phase and show a spinner while the
// target is generated — instead of freezing on LOBBY/SCORING. After this returns,
// room.referenceImageUrl is set unless the provider was hard-down for every
// attempt (last resort: proceed; scoreRoster forfeits all to 0). The CALLER opens
// the timed prompting window once this returns. Epoch-guarded throughout.
async function _prepareTopicAndReference(roomId, epoch) {
  const room = getRoom(roomId);
  if (!room || !room.tournament) return;
  const t = room.tournament;
  t.phase = TOURNAMENT_PHASES.ROUND_PROMPTING;
  // Enter the "preparing" state IMMEDIATELY (before awaiting nextTopic): clear any
  // prior target so a stale image is never shown, and broadcast PROMPTING with NO
  // duration → phaseEndsAt = null. Clients leave the prior phase (LOBBY/SCORING)
  // right away and show the spinner instead of freezing for the nextTopic latency.
  t.topic = null;
  room.referenceImageUrl = null;
  ...
  for (let attempt = 0; attempt < MAX_TOPIC_ATTEMPTS; attempt++) {
    try {
      ...
      if (!isCurrentEpoch(roomId, epoch)) return;
      log(roomId, `nextTopic failed (attempt ${attempt + 1}/${MAX_TOPIC_ATTEMPTS}): ${err.message}`);
      continue;
    }
    if (!isCurrentEpoch(roomId, epoch)) return;
    ...
    // Refresh the preparing broadcast (still no duration → phaseEndsAt null) so the
    // category/difficulty badge updates while the target generates.
    setPhase(roomId, PHASES.PROMPTING);
    broadcastState(roomId);
    await _generateReferenceImage(roomId, epoch);
    if (!isCurrentEpoch(roomId, epoch)) return;
    if (room.referenceImageUrl) break;
    log(roomId, `reference missing (attempt ${attempt + 1}/${MAX_TOPIC_ATTEMPTS}) — retrying with a fresh topic`);
  }
  if (!room.referenceImageUrl) {
    log(roomId, `LAST RESORT: no reference after ${MAX_TOPIC_ATTEMPTS} attempts — opening round anyway (all entrants forfeit to 0)`);
  }
}
...
async function _runGroup(roomId, epoch, groupIds, topK) {
  ...
  // 1) Topic + hidden target image (retry/replay + "preparing" broadcast), scoped
  //    to the group, THEN open the timed prompting window.
  await _prepareTopicAndReference(roomId, epoch);
  ...
  t.topic = null;
  room.referenceImageUrl = null; // fresh target next group/round
  broadcastState(roomId);
